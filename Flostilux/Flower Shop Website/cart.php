...
if(isset($_GET['delete'])){
    ...
    mysqli_query($conn, "DELETE FROM `cart` WHERE id = '$delete_id'") or die('Запрос не удался!');
...
if(isset($_POST['update_quantity'])){
    ...
    mysqli_query($conn, "UPDATE `cart` SET quantity = '$cart_quantity' WHERE id = '$cart_id'") or die('Запрос не удался!');
...
<section class="shopping-cart">
    ...
    ?>
    <div  class="box">
        <a href="cart.php?delete=<?php echo $fetch_cart['id']; ?>" class="fas fa-times" onclick="return confirm('Удалить это с корзины?');"></a>
        ...
        <div class="price"><?php echo $fetch_cart['price']; ?> Лей</div>
        ...
        <div class="sub-total"> Итого : <span><?php echo $sub_total = ($fetch_cart['price'] * $fetch_cart['quantity']); ?>Лей</span> </div>
    </div>
    <?php
    ...
    <div class="more-btn">
        <a href="cart.php?delete_all" class="delete-btn <?php echo ($grand_total > 1)?'':'disabled' ?>" onclick="return confirm('Вы уверены, что хотите удалить все с корзины?');">Удалить все</a>
    </div>

    <div class="cart-total">
        <p>Итого: <span><?php echo $grand_total; ?> Лей</span></p>
